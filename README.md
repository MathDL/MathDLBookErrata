...
6. Page 96: The expression of the last equality of the last display of the page should read $=w_{11}^{2}x_{1}^{1}+w_{21}^{2}x_{2}^{1}+ B^{2}.$ Namely $x_{2}^{2}$ there should be $x_{2}^{1}$.
7. Page 97: Towards the middle of the page, there should be a minus sign in the derivative of the formula for the loss function with respect to the model, i.e., it should be\
   $`\frac{\partial\Lambda}{\partial m}=-(y-m(x;\theta)).`$   
8. Page 219: Around the middle of the page, it should be $\mathfrak{m}(\textbf{X};\theta)$ instead of $\mathfrak{m}(\textbf{X},\theta).$
9. Page 267: In the statement of Lemma 16.13, second line, $\mathbb{R}_{1}$ should be $\mathbb{R}^{1}$.
10. Page 462: Equation (24.1) should read $z_{1}=\mathfrak{m}_{1}(x;\theta)$. There is an extra "=".
11. Page 462: Three lines before the end of the page there is a subscript $\ell-1$ missing on $z$ in the formula. The formula should read\
