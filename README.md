...
3. Page 95: The second formula from the end should read\
   $`\frac{\partial\Lambda}{\partial Z}=\frac{\partial\Lambda}{\partial H}\frac{\partial H}{\partial Z}=...`$   
4. Page 219: Around the middle of the page, it should be $\mathfrak{m}(\textbf{X};\theta)$ instead of $\mathfrak{m}(\textbf{X},\theta).$
5. Page 462: Equation (24.1) should read $z_{1}=\mathfrak{m}_{1}(x;\theta)$. There is an extra "=".
6. Page 462: Three lines before the end of the page there is a subscript $\ell-1$ missing on $z$ in the formula. The formula should read\
